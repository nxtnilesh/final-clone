        ...
        <DialogHeader className="p-4 border-b border-neutral-700">
          <div className="relative flex items-center">
            ...
            <Input
              ...
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-10 pr-10 py-2 rounded-lg bg-neutral-700 border-neutral-600 text-white placeholder:text-neutral-400 focus:ring-0 focus:ring-offset-0"
            />
            <Button
              variant="ghost"
              size="icon"
              onClick={() => onOpenChange(false)}
              className="absolute right-2 top-1/2 -translate-y-1/2 text-neutral-400 hover:bg-neutral-600 hover:text-white"
            >
              <X className="w-4 h-4" />
              <span className="sr-only">Close</span>
            </Button>
